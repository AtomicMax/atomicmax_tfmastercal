...
Includes school-wide events, breaks, holidays, special events, and Tallulah
Together. Excludes athletics and events that are Middle School, Lower School,
or grades 4-8 only.
"""
...
def is_ms_or_ls_only(title: str) -> bool:
    lowered = title.lower()
    hard_keep = any(re.search(pattern, lowered) for pattern in SCHOOLWIDE_OVERRIDE)
...
def parse_time_tag(value: str | None):
    if not value:
        return None
    try:
...
def extract_articles(html: str, source: str) -> list:
    ...
    for article in soup.select("article"):
        ...
        if all_day:
            ...
            events.append({"title": title, "start": start_dt, "end": end_dt, "all_day": True, "source": source, "occur_id": occur_id or ""})
        else:
...
def fetch_master_calendar(sess: requests.Session) -> list:
    events = []
    start_row = 1
    empty_pages = 0
    while start_row <= 2000:
        ...
        raw_count = sum(
            1 for link in raw_titles
            if "fsReadMoreLink" not in link.get("class", []) and clean_title(link.get_text(" ", strip=True))
        )
...
def event_uid(item: dict) -> str:
    key = "|".join([item["title"], item["start"].isoformat(), item["end"].isoformat(), item.get("occur_id", "")])
...
def strip_time_and_dates(title: str) -> str:
    ...
    text = re.sub(r"\b(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2}\b", " ", text)
...
def title_quality(item: dict):
    ...
    return (official, smore, junk, len(title))
...
def is_near_duplicate(a, b) -> bool:
    if not same_day(a, b):
        return False
    if a["title"] == b["title"]:
        return True
...
def title_with_time(title: str, start: datetime, end=None) -> str:
...
def all_day_event(title: str, day: date, source: str, occur_id: str = "", description: str = "") -> dict:
    ...
    return {"title": title, "start": start, "end": end, "all_day": True, "source": source, "occur_id": occur_id, "description": description}
...
def generate_ab_week_labels() -> list:
    events = []
    ...
    while monday <= last:
        ...
        for offset in range(5):
            ...
            if day < AB_PATTERN_START or day > AB_PATTERN_END:
                continue
            events.append(all_day_event(f"US {day_letter(day)} Day", day, source, occur_id=f"ab-day-{day.isoformat()}"))
        monday += timedelta(days=7)
    assert week_letter(AB_KNOWN_A_MONDAY) == "A"
    return events
...
def smore_usable_text(text: str) -> str:
    ...
    extra_chunks = []
    for heading in ("College Visits", "Club Day", "Important Events", "Reminders"):
...
def extract_smore_events(text: str, source: str) -> list:
    events = []
    ...
    for sentence in sentences:
        ...
        if re.match(r"^\d{1,2}(?::\d{2})?", title):
            continue
        interesting = bool(re.search(r"\b(college|university|ung|mercer|club|activity|activities|visit|assembly|invite|on campus|learning center|media center)\b", title, re.I))
        if not interesting:
            continue
        if len(title) > 140:
            title = title[:137] + "..."
...
def fetch_smore_events(sess: requests.Session) -> list:
    events = []
    for url in SMORE_URLS:
        try:
            ...
            if response.status_code != 200:
                ...
                continue
            found = extract_smore_events(smore_plain_text(response.text), url)
...
def format_early_release(title: str):
...
def run() -> None:
    ...
    for item in events[:25]:
        print(f"  [all-day] {item['start']} \u2014 {item['title']}")
    if len(events) > 25:
